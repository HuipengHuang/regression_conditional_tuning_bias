...
class LocalizedPredictor:
    ...
    def compute_cal_score(self, cal_loader):
        ...
        self.v_hat = torch.cat([torch.tensor(torch.tensor([-math.inf], device=self.device), device=self.device), cal_score, torch.tensor([math.inf], device=self.device)], dim=0)
