...
class NewWeightedTrainer:
    ...
    def __init__(self, args, num_classes):
        ...
        self.weight = nn.Parameter(torch.zeros(size=(3,), device=self.device))
        ...
        if args.adapter == "True":
            ...
            self.predictor = new_weighted_predictor.New_Weighted_Predictor(args, self.net, self.adapter.adapter_net)
        else:
            self.adapter = None
            self.predictor = new_weighted_predictor.New_Weighted_Predictor(args, self.net, self.weight, adapter_net=None)
